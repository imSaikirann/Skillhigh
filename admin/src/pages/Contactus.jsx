...
export default function Contactus() {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        const response = await axios.get('/api/v1/contacts/contactus'); // Adjust the endpoint if needed

        if (response.data.success) {
          setContacts(response.data.data); // Assuming `response.data.data` holds the contacts
        } else {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await axios.delete(`/api/v1/contacts/contactus/${id}`);
