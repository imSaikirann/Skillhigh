...
export default function Mentors() {
  ...
  const fetchMentors = async () => {
    try {
      const response = await axios.get('/api/v1/mentors/mentors');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
       const res =  await axios.put(`/api/v1/mentors/editMentor/${form.id}`, formData);
       setAlertMessage(res.data.message)
       setAlertVisible(true)
      } else {
        const res = await axios.post('/api/v1/mentors/addMentor', formData);
        setAlertMessage(res.data.message)
        setAlertVisible(true)
      }
      fetchMentors();
